...
Note that even when an **AST\_NUMBER**'s numType is **INT\_TYPE**, we will be storing it's actual value as a double under the hood. This is to make function evaluation a little less tedious; students wishing to instead use this alternative **AST\_NUMBER** are welcome to do so:
...
Each **AST\_NUMBER** will house a member of the **NUM\_TYPE** enum (denoting the numType of number being stored) alongside a numeric value.
...
We've already discussed two types of abstract syntax tree nodes for housing numbers and functions. In order to make these types of nodes interchangeable, it is necessary to wrap them in a generic abstract syntax tree node which can store either numType of node.

As such, we have the **AST\_NODE** numType:
...
An **AST\_NODE** stores a member of the **AST\_NODE\_TYPE** enum in it's **numType** field, so it can be determined what numType of node is housed in its **data** field (a union which can store either a **AST\_NUMBER** or a **AST\_FUNCTION**). It also stores a pointer to another **AST\_NODE** in its **next** field, which allows **AST_NODE**s to be stored in a linked list style for use as operands (see the **opList** field in **AST\_FUNCTION**).

## LEXING

First, it is necessary to define all tokens and all non-terminals within the grammar. **token**s (and non-terminals, called **numType**s by yacc), will be defined in [cilisp.y](../src/cilisp.y) in the definitions section. The provided token and numType definitions are:
...
The union contains all types that **token**s and **numType**s will have. In this case, **token** and **numType** values will all be stored as *double*, *int* or *ast_node \**.
...
```c
AST_NODE *createNumberNode(double value, NUM_TYPE numType);
...
For instance, in the `number ::= INT` production, you will likely want to assign `$$` (the value of the **number**) to the result of a call to **createNumberNode**, which should create an **AST\_NODE** housing an **AST\_NUMBER** whose data is populated with that from the **INT** token (i.e. its double value) and the numType (**INT\_TYPE**, from the **NUMBER\_TYPE** enum). Both **createNumberNode** and **INT\_TYPE** are accessible to the parser because they are declared in [cilisp.h](../src/cilisp.h).
...
The **eval** function takes as input a generic **AST\_NODE \***. Its job is to determine which numType of **AST\_NODE** is referrenced and call the corresponding function to evaluate it. That is, **eval** must determine if the referenced **AST\_NODE** is of numType **NUM\_NODE\_TYPE** or **FUNC\_NODE\_TYPE** and call **evalNumNode** or **evalFuncNode** on it respectively.
...
**evalFuncNode** should check which **FUNC_TYPE** the **AST\_FUNCTION** contains, and call an appropriate function to evaluate the result of running that operation with the input referenced linked list of operands. For instance, if the function being evaluated has numType **NEG_TYPE**, you might pass it off to a function called **evalNegFunction** (which you'll need to add).
...
* add
	...
	* return numType depends on input types
...
* sub
	...
	* return numType depends on operand types
* mult
	* *n*-ary
...
* max
	...
	* return numType is the numType of the maximum operand
	* if given no operands, prints a warning and returns `nan`
* min
	* *n*-ary
	* returns minimum value among the operands
	* return numType is the numType of the minimum operand
	* if given no operands, prints a warning and returns `nan`

## <a name="sample-runs"></a>SAMLPLE RUNS

In the sample runs below, inputs are on the same line as the `> `, and the output is printed below (along with any warnings). The inputs shown in these sample runs are all provided in the `inputs/task_1` directory. You can edit the run configurations to provide a path to an input file, or run without any arguments and numType inputs in by hand. The provided input files have `quit` so sample runs match runs done by hand, but this is unnecessary with file inputs; the program will end when an `EOF` is encounters as well.
