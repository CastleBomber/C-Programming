...
{
    char *identifier = node->leftChild->data.identifier;
    NUMBER value = evalExpr(node->rightChild);

    SYMBOL_TABLE_NODE *defintion = findSymbol(symbolTable, identifier);

    if(defintion == NULL){
        defintion = createSymbol(identifier, value);
        addSymbolToTable(&symbolTable, defintion);
    } else {
        reassignSymbol(defintion, value);
    }
}
...
void evalRepeatStmt(NODE *node)
{
    ...
    NUMBER value = evalExpr(node->leftChild);
...
{
    NUMBER value = evalTerm(node->leftChild);

    if (node->rightChild != NULL){
        NUMBER rightValue =  evalExpr(node->rightChild);
        value = evalOperation(value, rightValue, node->data.op);
    }
...
{
    NUMBER value = evalFactor(node->leftChild);

    if(node->rightChild != NULL){
        NUMBER rightValue = evalTerm(node->rightChild);
        value = evalOperation(value, rightValue, node->data.op);
    }
